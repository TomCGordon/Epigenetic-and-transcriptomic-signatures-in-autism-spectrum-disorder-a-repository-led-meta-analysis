...
# Download and stage the public repository inputs used for GSE4187.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE4nnn/GSE4187/matrix/GSE4187_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Lymphoblastoid_cell_lines/GSE4187/GSE4187_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "eabb54be1721487473471ef1ac49a5448c43d9e2fcf7506b581f34ba7640bb3b"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE4187_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE4187_file_check.csv"), row.names = FALSE, na = "")
