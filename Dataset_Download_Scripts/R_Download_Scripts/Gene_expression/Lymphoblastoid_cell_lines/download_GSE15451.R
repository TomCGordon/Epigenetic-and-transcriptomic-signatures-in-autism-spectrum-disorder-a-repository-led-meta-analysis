...
# Download and stage the public repository inputs used for GSE15451.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE15nnn/GSE15451/matrix/GSE15451_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Lymphoblastoid_cell_lines/GSE15451/GSE15451_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "ae3fe2ac8d3c079f036c83c514b15dfb26603f59089a0594b674656299570628"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE15451_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE15451_file_check.csv"), row.names = FALSE, na = "")
