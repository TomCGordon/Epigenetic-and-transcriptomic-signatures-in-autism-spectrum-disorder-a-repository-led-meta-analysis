...
# Download and stage the public repository inputs used for GSE15402.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE15nnn/GSE15402/matrix/GSE15402_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Lymphoblastoid_cell_lines/GSE15402/GSE15402_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE15402_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE15402_file_check.csv"), row.names = FALSE, na = "")
