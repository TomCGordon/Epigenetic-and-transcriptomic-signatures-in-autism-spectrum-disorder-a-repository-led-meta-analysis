...
# Download and stage the public repository inputs used for GSE36315.
...
downloads <- data.frame(
  ...
  destination = c(
    "01_Raw_Data/Gene_expression/Post_mortem_brain/GSE36315/GSE36315_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "473bffa72211ba0d95dedb900faaee6c00eb46c8f21ee362beaa8fa8e9fa3a5c"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE36315_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE36315_file_check.csv"), row.names = FALSE, na = "")
