...
# Download and stage the public repository inputs used for GSE59288.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE59nnn/GSE59288/suppl/GSE59288_exp_mRNA.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Post_mortem_brain/GSE59288/GSE59288_exp_mRNA.txt.gz"
  ),
  action = c(
    "none"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE59288_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE59288_file_check.csv"), row.names = FALSE, na = "")
