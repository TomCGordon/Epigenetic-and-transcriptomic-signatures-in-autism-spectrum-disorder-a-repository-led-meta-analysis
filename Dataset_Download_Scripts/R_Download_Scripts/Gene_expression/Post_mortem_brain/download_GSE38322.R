...
# Download and stage the public repository inputs used for GSE38322.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE38nnn/GSE38322/matrix/GSE38322_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Post_mortem_brain/GSE38322/GSE38322_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "489096bc9454484ce6b680a141ae77dd2612406ddb8cb885f5e94888fe3f2e67"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE38322_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE38322_file_check.csv"), row.names = FALSE, na = "")
