...
# Download and stage the public repository inputs used for GSE113834.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE113nnn/GSE113834/matrix/GSE113834_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Post_mortem_brain/GSE113834/GSE113834_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "36ee2b7c5488786e4f0760fa4a924bfc9637bc24958126761576c66866735f2a"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE113834_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE113834_file_check.csv"), row.names = FALSE, na = "")
