...
# Download and stage the public repository inputs used for GSE211154.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE211nnn/GSE211154/suppl/GSE211154_counts.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Post_mortem_brain/GSE211154/GSE211154_counts.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "b1085c8f884884a457bd52e19d57dfa5714259efd27b56007f6aea3795406252"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE211154_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE211154_file_check.csv"), row.names = FALSE, na = "")
