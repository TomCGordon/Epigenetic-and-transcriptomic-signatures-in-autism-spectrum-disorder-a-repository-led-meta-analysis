...
# Download and stage the public repository inputs used for GSE236761.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE236nnn/GSE236761/suppl/GSE236761_raw_counts.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Post_mortem_brain/GSE236761/GSE236761_raw_counts.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "ff6e1ffaf00faf87e0f334ca4db21c28c1038e5edf8d3906dfc8c9ed1919b52b"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE236761_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE236761_file_check.csv"), row.names = FALSE, na = "")
