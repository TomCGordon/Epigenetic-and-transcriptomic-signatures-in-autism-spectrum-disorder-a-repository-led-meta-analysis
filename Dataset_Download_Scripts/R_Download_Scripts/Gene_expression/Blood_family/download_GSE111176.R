...
# Download and stage the public repository inputs used for GSE111176.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE111nnn/GSE111176/matrix/GSE111176_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Blood_family/GSE111176/GSE111176_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "ea8de1a4aa8497ddc575c8f54e66591fde2a947ae057057e9f069f5f06acb348"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE111176_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE111176_file_check.csv"), row.names = FALSE, na = "")
