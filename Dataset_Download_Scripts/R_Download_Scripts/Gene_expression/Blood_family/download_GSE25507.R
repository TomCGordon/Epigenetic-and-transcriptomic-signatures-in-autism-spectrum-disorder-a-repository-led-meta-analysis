...
# Download and stage the public repository inputs used for GSE25507.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE25nnn/GSE25507/matrix/GSE25507_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Blood_family/GSE25507/GSE25507_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "ec48890c4de3be2901c8f2170a3d2fe10e9957d402c39642c57cffa609708882"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE25507_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE25507_file_check.csv"), row.names = FALSE, na = "")
