...
# Download and stage the public repository inputs used for GSE123302.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE123nnn/GSE123302/matrix/GSE123302_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/Gene_expression/Blood_family/GSE123302/GSE123302_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "8289b441b984218a3b9b9c8a8efc54b7331972306d6db3d9e996775ddec647d2"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE123302_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE123302_file_check.csv"), row.names = FALSE, na = "")
