...
# Download and stage the public repository inputs used for GSE80017.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE80nnn/GSE80017/matrix/GSE80017_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/DNA_methylation/Post_mortem_brain/GSE80017/arrays/GSE80017_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "3e50583a4a35a059567253bc3122f456c49020ff0eed80a1fd40287a165a2aaa"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE80017_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE80017_file_check.csv"), row.names = FALSE, na = "")
