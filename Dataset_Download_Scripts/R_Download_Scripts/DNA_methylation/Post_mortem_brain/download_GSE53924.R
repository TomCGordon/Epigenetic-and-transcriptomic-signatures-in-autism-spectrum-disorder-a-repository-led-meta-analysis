...
# Download and stage the public repository inputs used for GSE53924.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE53nnn/GSE53924/matrix/GSE53924_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/DNA_methylation/Post_mortem_brain/GSE53924/arrays/GSE53924_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "956cf8fab577494c3317a5042fa63f25c8f7352844305959ad8427b5dbe6ec10"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE53924_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE53924_file_check.csv"), row.names = FALSE, na = "")
