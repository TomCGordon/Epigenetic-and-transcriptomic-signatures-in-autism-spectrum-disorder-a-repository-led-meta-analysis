...
# Download and stage the public repository inputs used for GSE38608.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE38nnn/GSE38608/matrix/GSE38608_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/DNA_methylation/Post_mortem_brain/GSE38608/arrays/GSE38608_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE38608_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE38608_file_check.csv"), row.names = FALSE, na = "")
