...
# Download and stage the public repository inputs used for GSE113967.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE113nnn/GSE113967/matrix/GSE113967_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/DNA_methylation/Blood_family/GSE113967/GSE113967/GSE113967_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "b4528942704af8a4f2d86124bf5ee990df50a0457cfdf54f2f1be765af33a111"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE113967_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE113967_file_check.csv"), row.names = FALSE, na = "")
