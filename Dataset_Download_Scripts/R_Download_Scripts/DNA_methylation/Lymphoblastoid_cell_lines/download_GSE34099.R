...
# Download and stage the public repository inputs used for GSE34099.
...
downloads <- data.frame(
  remote_url = c(
    "https://ftp.ncbi.nlm.nih.gov/geo/series/GSE34nnn/GSE34099/matrix/GSE34099_series_matrix.txt.gz"
  ),
  destination = c(
    "01_Raw_Data/DNA_methylation/Lymphoblastoid_cell_lines/GSE34099/lcl/GSE34099_series_matrix.txt.gz"
  ),
  action = c(
    "none"
  ),
  ...
  expected_sha256 = c(
    "4fd57e066b3c1fafb0d1e69653016cd5681b4810eed463599b98a55cd16297ca"
  ),
...
if (!dry_run) {
  ...
  utils::write.csv(do.call(rbind, log_rows), file.path(log_dir, "GSE34099_download_log.csv"), row.names = FALSE, na = "")
  utils::write.csv(checks, file.path(log_dir, "GSE34099_file_check.csv"), row.names = FALSE, na = "")
